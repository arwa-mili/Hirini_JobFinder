    ...
    <div className='bg-black'>
      ...
      >
        <div className='w-full z-10'>
          <div className='mb-8'>
            <p className='text-yellow-500 font-bold text-4xl'>{title}</p>
          </div>
          ...
          <div className='w-full flex items-center justify-around bg-white px-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full'>
            ...
            <div>
              <CustomButton
                ...
                title='Search'
                containerStyles={
                  "text-white py-2 md:py3 px-3 md:px-10 focus:outline-none bg-yellow-500 rounded-full md:rounded-md text-sm md:text-base"
                }
              />
            </div>
          </div>
          ...
          {type && (
            <div className='w-full lg:1/2 flex flex-wrap gap-3 md:gap-6 py-10 md:py-14'>
              {popularSearch.map((search, index) => (
                <span
                  key={index}
                  className='bg-yellow-500 text-black py-1 px-2 rounded-full text-sm md:text-base'
                >
                  {search}
                </span>
              ))}
        ...
        <div className='w-1/3 h-full absolute top-24 md:-top-6 lg:-top-14 right-16 2xl:right-[18rem]'>
          <img src={Jung} className='object-contain' />
        </div>
      </div>
    </div>
